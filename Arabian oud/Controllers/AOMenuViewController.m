...
@interface AOMenuViewController () <UIAlertViewDelegate>

@property (nonatomic, retain) NSMutableArray *itemsInTable;

@end

@implementation AOMenuViewController
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if ([[GlobalData sharedGlobalData].g_userInfo.signup isEqualToString:USER_LOGIN]) {
        ...
        m_lblUserName.text = [GlobalData sharedGlobalData].g_userInfo.firstName;
    } else {
...
- (UITableViewCell*)createCellWithTitle:(NSString *)title isCheck:(NSString *)isCheck indexPath:(NSIndexPath*)indexPath
{
    if ([[[self.itemsInTable objectAtIndex:indexPath.row] valueForKey:@"level"] intValue] == 0) {
        ...
        [m_btnMainCategory setTitle:[title uppercaseString] forState:UIControlStateNormal];
...
- (void)onGotoPerfumes {
    ...
    [[GlobalData sharedGlobalData].g_mainCtrl.leftMenuView hidenWithAnimation];
}
